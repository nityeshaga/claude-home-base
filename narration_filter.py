...
_HOLDING_RES = tuple(
    ...
    for p in (
        ...
        rf"^(?:still\s+)?waiting\s+(?:on|for)\s+(?:the\s+)?(?:{_COUNT}\s+)?"
        rf"(?:remaining\s+|last\s+|final\s+)?{_INTERNAL_UNIT}\b.*$",
        ...
        rf"^(?:{_COUNT}\s+)?{_INTERNAL_UNIT}[\w\s,\-–]{{0,24}}\b"
        r"(?:left|remaining|to go|outstanding)\b.*$",
...
def is_content_free_holding(text_block: str) -> bool:
    ...
    if _SUBSTANTIVE_RE.search(stripped):
        return False
    return any(r.match(stripped) for r in _HOLDING_RES)
